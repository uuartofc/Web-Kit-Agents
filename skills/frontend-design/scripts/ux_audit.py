...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Familiar patterns
        if has_form:
            has_standard_labels = bool(re.search(r'<label|placeholder|aria-label|label\s*=\s*["\'{]', content, re.IGNORECASE))
        ...
        for purple in purple_hexes:
            if purple.lower() in content.lower():
                self.warnings.append(f"[Color] {filename}: PURPLE DETECTED ('{purple}'). Banned by default Maestro rules, but allowed here due to client requirements.")
                break
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'docs', 'certs', 'scripts', 'scratch', 'gen'}]
            for file in files:
