...
Before considering a UI complete, verify mobile and desktop widths:

- keyboard navigation has a visible focus state and logical order;
- controls have accessible names, usable touch targets, and loading/error states;
- text remains readable without overlap, clipping, or horizontal scrolling;
- contrast, headings, labels, alt text, reduced motion, and errors meet the project target;
- layout remains stable while content loads and during validation errors;
- images use an intentional loading strategy and avoid layout shift;
- animations simplify under reduced motion and use transform/opacity where possible;
- primary workflows are testable with semantic selectors, not only snapshots.

Run the available UX/accessibility audits after implementation and fix findings caused by the change before unrelated polish.
